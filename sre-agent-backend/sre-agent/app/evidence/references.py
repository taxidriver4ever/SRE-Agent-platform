...
class SourceReference(BaseModel):
    """指向 Conversation Store 中原始 Tool Result 的外部来源。"""

    kind: Literal["kubernetes", "git", "metrics", "logs", "trace", "database"]
    uri: str
    label: str
...
def build_source_references(
    ...
    *,
    ...
    if tool_name in {"list_pods", "list_deployments", "get_pod", "get_pod_events", "get_restart_count", "get_container_image", "get_deployment"}:
        ...
        return [SourceReference(kind="kubernetes", uri=f"k8s://{quote(namespace)}/{quote(tool_name)}/{quote(name)}", label=f"Kubernetes {namespace}/{name}")]
    if tool_name in {"read_file", "read_file_at_commit", "get_commit", "get_commit_diff", "list_changed_files", "search_code", "get_previous_commit", "get_repository", "get_current_commit"}:
        ...
        return [SourceReference(kind="git", uri=f"git+{base_uri}@{quote(commit, safe='')}{suffix}", label=f"{repository}@{commit[:12]}{('/' + path) if path else ''}", repository_url=repository_url, commit=commit, path=path)]
