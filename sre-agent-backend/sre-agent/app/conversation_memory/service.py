    ...
    async def maybe_compact(
        ...
        *,
        ...
        messages = prompt_messages(
            ...
            json.dumps(input_document, ensure_ascii=False, default=str),
        )
        template = CompactionOutput(
            conversation_summary="", context_state=ShortContextState(), memory_items=[],
        ).model_dump(mode="json")
        result = await generate_structured(
            self.llm, messages, CompactionOutput, retries=self.structured_output_retries,
            template=template, empty_response="", first_nonempty=False,
        )
        output = result.value
        if output is None:
            self.last_failed_raw_output[conversation_id] = result.original_output
            return None
