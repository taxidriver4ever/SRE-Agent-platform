...
测试使用独立 MySQL 测试库，覆盖明文不落库、模块化建表、Bearer 鉴权、
...
@pytest.fixture
def client(gateway_database):
    """为每个测试创建使用独立 MySQL 测试库的客户端。"""
    app = create_app(gateway_database)
    with TestClient(app) as test_client:
        yield test_client, app.state.token_service, gateway_database


def test_generate_token_stores_only_hash(client):
    """服务生成的 Token 格式正确，并且 MySQL 记录不含明文。"""
    _, service, database = client
    generated = service.generate()

    assert TOKEN_PATTERN.fullmatch(generated.token)
    with database.engine.connect() as connection:
        row = connection.execute(text("SELECT token_hash FROM gateway_tokens")).fetchone()

    assert row == (hash_token(generated.token),)
    assert len(row[0]) == 64
    assert generated.token != row[0]
...
def test_auth_module_creates_its_gateway_tokens_table(client):
    ...
    with database.engine.connect() as connection:
        columns = connection.execute(text(
            ...
            "WHERE table_schema=:schema AND table_name='gateway_tokens' ORDER BY ORDINAL_POSITION"
        ), {"schema": database.database_name}).fetchall()
