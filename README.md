...
Argo CD 负责实际同步、漂移修复和健康状态；staging/prod 通过部署仓库评审后逐级推广。

```mermaid
flowchart LR
  G[Application Git] --> CI[Tests / Agent Regression / Security]
  CI --> B[Build → Image Scan → GHCR SHA + Digest]
  B --> D[Independent Deployment Git]
  D --> A[Argo CD → Helm Render]
  A --> K[Kind: sre-dev / sre-staging / sre-prod]
  R[Git Revert] --> D
```

应用 CI 不再持有集群凭据，不运行集群部署或命令式回滚；旧 `cd.yml` 已删除，
旧 `deploy-local-k8s.sh` 明确返回废弃提示。原生应用 YAML 已转为 Helm 模板；
实验环境的 Compose 和 Lab 故障注入脚本保留，它们不属于生产发布通道。

完整修改清单、独立仓库导出、Secrets、Kind 引导、发布/回滚演练、验证结果与边界，见
[GitOps 交付说明](docs/gitops-delivery.md)。`deploy/gitops-template/` 仅为初始化模板，
真实环境状态只保存在导出后的独立 Deployment Repository，Argo CD 不跟踪应用仓库。
