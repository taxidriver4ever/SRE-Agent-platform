...
`./scripts/run-scenario.ps1 -Scenario SRE-001` 到 `SRE-010`。每次脚本都会先调用 `reset-lab.ps1`，也可单独运行该脚本恢复 normal。停止集群执行 `stop-lab.ps1`；完全重建执行 `deploy-lab.ps1`。

SRE-008 只劣化一个稳定 Pod；SRE-009 让同一个 Service 同时路由到两个 GOOD Pod 和一个 BAD canary Pod；SRE-010 注入错误 liveness 路径。三者分别覆盖单实例、混合版本与健康检查配置故障。
